...
// --- MEMBERSHIP & FEE VERIFICATION API ---
export const membershipApi = {
  getAll: async (params = {}) => {
    const searchParams = new URLSearchParams();
    if (params.status && params.status !== 'ALL') searchParams.append('status', params.status);
    if (params.class && params.class !== 'ALL') searchParams.append('class', params.class);
    if (params.payment_mode && params.payment_mode !== 'ALL') searchParams.append('payment_mode', params.payment_mode);
    if (params.search) searchParams.append('search', params.search);
    if (params.page) searchParams.append('page', params.page);
    if (params.limit) searchParams.append('limit', params.limit);
    if (params.sort_by) searchParams.append('sort_by', params.sort_by);
    const qs = searchParams.toString() ? `?${searchParams.toString()}` : '';
    const res = await request(`/membership${qs}`);
    return res.data;
  },
  getStats: async () => {
    const res = await request('/membership/stats');
    return res.data;
  },
  getById: async (id) => {
    const res = await request(`/membership/${id}`);
    return res.data;
  },
  create: async (membershipData) => {
    const isFormData = membershipData instanceof FormData;
    const res = await request('/membership', {
      method: 'POST',
      body: isFormData ? membershipData : JSON.stringify(membershipData),
    });
    return res.data;
  },
  verify: async (id, verifyPayload) => {
    const res = await request(`/membership/${id}/verify`, {
      method: 'PATCH',
  ...
  bulkImport: async (records) => {
    const res = await request('/membership/bulk-import', {
      method: 'POST',
      body: JSON.stringify({ records }),
    });
    return res.data;
  },
  importLocalSheet: async (filePath) => {
    const res = await request('/membership/import-local-sheet', {
      method: 'POST',
